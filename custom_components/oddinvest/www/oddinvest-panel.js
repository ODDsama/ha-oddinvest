...
const INFO = {
  ...
  forecast: ["Скільки буде на дедлайн", "Усі суми — на ОДНУ дату (твій дедлайн) і в гривні СЬОГОДНІШНЬОЇ купівельної спроможності, тому їх можна порівнювати між собою і з ціллю, яку ти задав сьогодні. Перемикач ₴/$ нічого не перераховує: це та сама величина, поділена на сьогоднішній курс. У доларах рядок «номінально» зникає — долар і є реальна одиниця, тож у ньому номінальна й реальна суми збігаються. Три верхні рядки описують РИНОК: внесок у всіх трьох плановий, а відрізняються вони довгостроковою ставкою (±3 п.п.) і темпом знецінення гривні (±4 п.п.). Рядок «За фактом» описує ТЕБЕ: ринок той самий, що в реалістичному, але замість планового внеску береться твій фактичний темп поповнень — тож різниця між цими двома рядками це рівно твоя дисципліна, без домішки ринку. Він зʼявляється з першого ж поповнення, а поруч написано, на якій довжині історії порахований темп, — за один місяць це радше здогад, за пів року вже показник. Запис «₴ 15.7% → 11.0%» означає, що ставка не вічна: сьогоднішня — це факт, а далі вона лінійно сповзає до довгострокової. Кожна валюта рахується окремо у своїй валюті: гривневий рукав наприкінці переводиться в сьогоднішні гроші й тому втрачає, а долар і євро купівельну спроможність тримають."],
};
...
class OddInvestPanel extends HTMLElement {
  ...
  _goalsHTML() {
    ...
    // «За фактом» відокремлюємо від ринкового віяла рискою: він відповідає
    // на інше питання (що з твоєю дисципліною), і змішувати його зі
    // сценаріями ринку означало б порівнювати різні речі як однорідні.
    const scenarioRow = (r) => {
      const pct = Math.max(0, Math.min(100, (r.amount / scale) * 100));
      // Для фактичного темпу підписуємо, на якій довжині історії він
      // порахований: за один місяць це радше здогад, ніж показник.
      ...
      // «за N років» чіпляємо до тієї валюти, яка справді сповзає, а не в
      // кінець списку — інакше читається так, ніби стосується й долара.
    ...
    if (goal > 0) {
      ...
      goalBlock = `<div style="border-top:1px solid var(--divider-color,#3334);padding-top:8px;margin-top:4px">
        ...
        ${actual ? `<div class="muted" style="font-size:12px;margin-top:2px">за фактичним темпом: ${
          actual.goal_months === -1 ? "вже досягнуто"
          : actual.goal_months > 0 ? `${monthYear(actual.goal_date)} · через ${humanMonths(actual.goal_months)}`
          : "не досягається за 60 років"}</div>` : ""}
        ${need}</div>`;
    }
