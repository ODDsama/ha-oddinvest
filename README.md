...
Контракт покриває всі види, які веде застосунок: ОВДП, сертифікати
фондів, строкові вклади, НПФ і резерв. Сутності при цьому зведені —
капітал, дохідність і ліквідність приходять уже змішаними по видах, бо
саме так на них і дивляться з дашборда.
...
Третє поле, **токен REST**, потрібне лише коли сервіс закритий паролем
(README `oddinvest` → «Доступ ззовні»). Береться в самому застосунку:
«Налаштування → Доступ ззовні → Видати токен». Без нього закритий сервіс
відповідає 401 на кнопку «Оновити дані», повзунки цілей і «Отримано» у
сповіщенні. Дописати його до наявного запису — «Переналаштувати» на картці
інтеграції, видаляти й ставити наново не треба.

**Публічної адреси серед полів немає навмисно.** Її задає сам сервіс, коли
власник підключає тунель, і публікує в документі стану
(`settings.public_url`) — звідти її беруть посилання у сповіщеннях і картка
пристрою. Друге поле для тієї самої адреси було б другим джерелом однієї
правди й застаріло б рівно тоді, коли адресу міняли. REST при цьому й далі
ходить у локальну адресу: за зовнішньою може стояти другий замок
(Cloudflare Access), якого машина не пройде.

## Сутності

Сорок одна: 27 `sensor`, 8 `binary_sensor`, 3 `number`, по одному
`date`, `button` і `calendar`. Префікс `entity_id` — `odd_invest_` (від імені пристрою
...
| `net_worth_uah` | ЧИСТИЙ капітал: капітал мінус усе, що винен, включно з пільговим боргом картки; без боргів дорівнює капіталу |
| `capital_delta_30_uah` | на скільки змінився капітал за 30 днів проти добового знімка; атрибут `contributed_uah` — скільки з цього внесено ззовні, і без нього дельта читалась би як заробіток |
| `idle_cost_month_uah` | ПРОСТІЙ: що коштує на місяць лежання вільних грошей брокера, на які квиток уже є, за сьогоднішньою порадою (контракт: `idle` — факт, `idle_cost` — ціна); атрибути `investable_uah` (скільки лежить понад квиток), `since`/`days` (відколи), `rate_pct`/`rate_label` (звідки ставка). `unknown` = простою немає; нуль = лежить, але поради для цих грошей немає |

**Борг**

| Ключ | Що показує |
|---|---|
| `debt_total_uah` | борг ПІД СТАВКОЮ (розстрочки й непільгова частина картки); пільговий оборот картки сюди не входить, тож нуль при живій картці — відповідь, а не пропуск; атрибути `top_name`, `top_rate_pct` |
| `card_bring_by_due_uah` | скільки принести на картки до розрахункової дати, щоб відсотків не було; атрибути: `nearest`, `nearest_min_uah` і таблиця `cards` (для кожної — дата, днів, мінімум, вільно, вік звірки) |
| `card_days_to_due` | днів до найближчої розрахункової дати серед звірених карток |
| `card_due_date` | сама дата |
...
| `usd_share_pct` / `eur_share_pct` | валютні частки капіталу; атрибути `rate` і `windows` — де стоїть сьогоднішній курс серед історії (перцентиль за 1/3/10 років) |

**Що робити** (`sensor`)

| Ключ | Що показує |
|---|---|
| `tasks` | скільки задач у черзі; атрибути: `now`/`soon`/`watch`, `top_title`, `why`, `action`, `kind` і повний масив `tasks` |

Черга приходить із сервіса вже впорядкованою — сортувати її на боці HA не
треба й не можна: порядок задає та сама логіка, що малює головну сторінку
застосунку, і друге впорядкування розвело б два екрани.
...
| `card_due_soon` | до розрахункової дати картки ≤ 7 днів і принести ще є що; гасне сам, щойно платіж закриє суму; атрибути: `name`, `due_date`, `bring_by_due_uah` | `problem` |
| `card_mark_stale` | хоч одна картка звірена понад 14 днів тому або не звірена зовсім; атрибут `stale` — їхні назви | `problem` |
...
content — це та ціна, яку раніше платила за нас проксі. Відколи сервіс
має публічну https-адресу (тунель), в iframe можна вписати її — тоді
mixed content немає, а вхід у застосунок питає його власний пароль.
