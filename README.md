...
Два необовʼязкові поля — для сервіса, закритого паролем і виведеного
назовні (README `oddinvest` → «Доступ ззовні»):

- **Токен REST** — значення `ODDINVEST_AUTH_TOKEN` сервіса. Без нього
  закритий сервіс відповідає 401 на кнопку «Оновити дані», повзунки цілей
  і «Отримано» у сповіщенні.
- **Публічна адреса** — `https://…`, з якої застосунок відкривається з
  телефона. Іде ЛИШЕ в посилання сповіщень і картку пристрою; REST
  інтеграція й далі ходить у локальну адресу — зовнішній вхід може стояти
  за другим замком (Cloudflare Access), якого машина не пройде.

Дописати їх до наявного запису — «Переналаштувати» на картці інтеграції,
видаляти й ставити наново не треба.
