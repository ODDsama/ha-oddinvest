...
def test_capital_uah_parsed():
    ...
    doc = StateDoc.from_payload(load("basic.json"))
    # 138 246.80 номіналу + 60 000 резерву + 30 000 цілей + 45 000 НПФ.
    #
    # Число переїжджало ДВІЧІ, і обидва рази не регресією фікстури, а тим,
    # заради чого сутність і заводили на боці сервіса: з 198 246.80 на
    # 243 246.80, коли в капітал увійшов НПФ, і звідти на 273 246.80, коли
    # ввійшли цілі накопичення. Доти будь-яка частка при наявних цілях була
    # завищеною.
    assert doc.capital_uah == 273246.8
    assert doc.capital() == 273246.8
